...
import { BaseLayout, GridOptions } from "./WidgetLayout";
...
function placeWidget(container: Container, grid: BaseLayout, widget: IWidget, col: number, row: number, isStatic?: boolean): void {
    const src = widget.getModel() as Record<string, unknown>;
    const copy: Record<string, unknown> = { ...src };
    const cell = grid.getLayout(col, row);
    copy.layout = { x: cell.x, y: cell.y, w: cell.w, h: cell.h, static: isStatic != undefined ? isStatic : true };
    container.compilation.widgets.push(copy as WebStudioWidget);
}
...
export class HContainer {
    protected readonly grid: BaseLayout;
    protected readonly container: Container;

    constructor(options: HContainerOptions) {
        this.grid = new BaseLayout({ ...options, rows: [1] });
...
export class VContainer implements IWidget {
    ...
    constructor(options: VContainerOptions) {
        this.grid = new BaseLayout({ ...options, columns: [1] });
        this.container = new Container();
        this.container.compilation.setLayout(this.grid.modelOptions);
    }
...
export class GridContainer implements IWidget {
    ...
    constructor(options: GridOptions) {
        this.grid = new BaseLayout(options);
        this.container = new Container();
        this.container.compilation.setLayout(this.grid.modelOptions);
    }
